...
function SVGCurvedArrow({
  ...
  const curveProps = {
    ...
    centerXY: [
      (clientWidth - pointerLength) / 2 + (clientWidth - pointerLength) * cp.centerXY[0],
      (clientHeight - pointerLength) / 2 + (clientHeight - pointerLength) * cp.centerXY[1],
    ],
    curveXY: [clientWidth / 3 + (clientWidth / 3) * cp.curveXY[0], 10 + (clientHeight / 3) * cp.curveXY[1]],
    strokeWidth: 3,
    strokeColor: '#f7f7f7',
    customStyles: {},
  };
